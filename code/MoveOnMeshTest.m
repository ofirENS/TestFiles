...
rPos = FindCrossection(x0,x1,vertices);
...
function rPos = FindCrossection(x0,x1,vertices)
...
% z coordinate sign of mesh triangles' vertices after rotation
...
cPoints        = zeros(numel(sNewPos),3);

for sIdx = 1:numel(sNewPos)/3
    inds  = ((sIdx-1)*3 +1):sIdx*3;
    signs = sNewPos(inds);
    sameSign(sIdx) = sum(signs==signs(1))==3;
    ...
    triIndicator =(t>=0 & t<=1);% where there is intersection
    ...
    p       = p(triIndicator,:);
    cPoints(inds(triIndicator),:) = p;% [cPoints;p(1,:);p(2,:)];
%     vertIndicators(inds(~triIndicator))=false;
    end
end
cPoints = cPoints(vertIndicators,:);
toc
...
% Arrange the indices of the resulting curve, discard any internal
% structures 
