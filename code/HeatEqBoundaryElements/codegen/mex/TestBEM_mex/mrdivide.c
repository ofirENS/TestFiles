...
static emlrtRSInfo hf_emlrtRSI = { 107, "eml_lusolve",
  "D:\\Program Files\\MATLAB\\R2014a\\toolbox\\eml\\lib\\matlab\\eml\\eml_lusolve.m"
};
  ...
  emxArray_real_T *B, emxArray_real_T *Y)
{
  ...
  boolean_T exitg1;
  const mxArray *y;
  static const int32_T iv76[2] = { 1, 8 };

  const mxArray *m12;
  char_T cv71[8];
  static const char_T cv72[8] = { '%', '%', '%', 'd', '.', '%', 'd', 'e' };
  ...
  if (mn > 0) {
    ...
    while ((!exitg1) && (k <= mn - 1)) {
      if (muDoubleScalarAbs(b_A->data[k + b_A->size[0] * k]) <= xnorm) {
        st.site = &lc_emlrtRSI;
        y = NULL;
        m12 = emlrtCreateCharArray(2, iv76);
        for (i = 0; i < 8; i++) {
          cv71[i] = cv72[i];
        }

        emlrtInitCharArrayR2013a(&st, 8, m12, cv71);
        emlrtAssign(&y, m12);
        b_st.site = &tg_emlrtRSI;
        emlrt_marshallIn(&b_st, c_sprintf(&b_st, b_sprintf(&b_st, y,
          emlrt_marshallOut(14.0), emlrt_marshallOut(6.0), &o_emlrtMCI),
          emlrt_marshallOut(xnorm), &p_emlrtMCI), "sprintf", cv73);
        st.site = &kc_emlrtRSI;
        b_eml_warning(&st, atmp, cv73);
        exitg1 = true;
      } else {
        atmp++;
        k++;
...
{
  const mxArray *y;
  static const int32_T iv92[2] = { 1, 21 };

  const mxArray *m29;
  char_T cv79[21];
  int32_T i;
  static const char_T cv80[21] = { 'C', 'o', 'd', 'e', 'r', ':', 'M', 'A', 'T',
    'L', 'A', 'B', ':', 'd', 'i', 'm', 'a', 'g', 'r', 'e', 'e' };

  emxArray_real_T *b_B;
  emxArray_real_T *b_A;
  emxArray_int32_T *ah_emlrtRSI;
  int32_T loop_ub;
  real_T A_data[1];
  real_T alpha1;
  char_T DIAGA;
  ...
  if (B->size[1] == A->size[1]) {
    ...
    y = NULL;
    m29 = emlrtCreateCharArray(2, iv92);
    for (i = 0; i < 21; i++) {
      cv79[i] = cv80[i];
    }

    emlrtInitCharArrayR2013a(sp, 21, m29, cv79);
    emlrtAssign(&y, m29);
    st.site = &hb_emlrtRSI;
  ...
  b_emxInit_int32_T(sp, &ah_emlrtRSI, 2, &k_emlrtRTEI, true);
  if ((A->size[1] == 0) || (B->size[1] == 0)) {
    ...
    eml_signed_integer_colon(&i_st, 1, ah_emlrtRSI);
    i = 0;
              ...
              emxArray_real_T *B, emxArray_real_T *y)
{
  ...
  if (B->size[1] == A->size[1]) {
    ...
    b_y = NULL;
    m11 = emlrtCreateCharArray(2, iv75);
    for (i = 0; i < 21; i++) {
      cv69[i] = cv70[i];
    }

    emlrtInitCharArrayR2013a(sp, 21, m11, cv69);
    emlrtAssign(&b_y, m11);
    st.site = &hb_emlrtRSI;
