...
diagEqn =  @(N,nc,n) nchoosek(Nl -(N-3),nc-n).*nchoosek(N-3,n)./nchoosek(Nl, nc);
...
for alphaIdx = 1:numAlphas
 ...
 numConnectors = floor(0.01*alphaIdx*Nl);
...
for expIdx = 1:numExperiments
    z     = zeros(1,Nl);%(N-1)*(N/2-1));
    rp    = randperm(Nl);%(N-1)*(N/2 -1));
...
meanGsTheoretical(alphaIdx)            = (N-3)*numConnectors/Nl;
meanGsTheoreticalEndMonomers(alphaIdx) = (N-2)*numConnectors/Nl;
...
temp(diag(true(1,N),0)) = meanGsTheoretical(alphaIdx);%-sum(temp,2);
temp(1,1)       = meanGsTheoreticalEndMonomers(alphaIdx);
temp(end,end)   = meanGsTheoreticalEndMonomers(alphaIdx);
B(:,:,alphaIdx) = temp;
[0 numConnectors/Nl *N +rouseEigenVal(2:end).*(1-numConnectors/Nl)];
end
